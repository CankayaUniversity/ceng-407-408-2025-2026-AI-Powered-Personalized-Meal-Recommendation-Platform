...
public class IngredientController {
    ...
    public List<UnitConversionDTO> getConversions(
        ...
        String normalizedInputUnit = unitConverterService.normalizeUnit(unit);

        unitWeights.forEach((targetUnit, weight) -> {
            if (weight > 0) {
                String lowerTarget = targetUnit.toLowerCase();
                String normalizedTarget = unitConverterService.normalizeUnit(targetUnit);
                
                // Hangi gruba ait olduğunu bul
                String groupKey = unitGroups.entrySet().stream()
                        .filter(e -> e.getValue().contains(lowerTarget) || e.getValue().contains(normalizedTarget))
                ...
                boolean isPreferred = ingredient.getPreferredUnit() != null && 
                                     unitConverterService.normalizeUnit(targetUnit).equals(unitConverterService.normalizeUnit(ingredient.getPreferredUnit()));
                
                boolean isCustom = ingredient.getIngredientUnits() != null && 
                                  ingredient.getIngredientUnits().stream()
                                          .anyMatch(iu -> unitConverterService.normalizeUnit(iu.getUnitName()).equals(normalizedTarget));
    ...
    private boolean isHighPriorityUnit(String unit, Ingredient ingredient, boolean isPreferred, boolean isCustom) {
        ...
        if (category == Ingredient.Category.BEVERAGE || 
           (category == Ingredient.Category.DAIRY && state == Ingredient.PhysicalState.LIQUID)) {
            return List.of("ml", "l", "lt", "litre", "su bardagi", "bardak", "glass").contains(lowerUnit);
        }

        // 2. Soslar ve Yağlar
        if (category == Ingredient.Category.SAUCE || category == Ingredient.Category.OIL) {
            return List.of("ml", "l", "lt", "yemek kasigi", "tablespoon", "su bardagi", "bardak").contains(lowerUnit);
        }

        // 3. Baharatlar ve Tatlandırıcılar
        if (category == Ingredient.Category.SPICE || category == Ingredient.Category.SWEETENER) {
            return List.of("g", "gram", "cay kasigi", "teaspoon", "tatli kasigi", "tutam", "pinch").contains(lowerUnit);
        }
    ...
    private boolean isVolumeUnit(String unit) {
        return List.of("ml", "litre", "liter", "l", "lt", "glass", "bardak", "su bardagi", "cay bardagi", "kahve fincani",
                "tablespoon", "yemek kasigi", "teaspoon", "tatli kasigi", "cay kasigi", "cup", "bowl", "kase",
                "fincan", "kepce", "kasik", "dal", "demet", "dis", "clove", "tutam", "pinch", "avuc", "handful").contains(unit.toLowerCase());
    }
    ...
    private int getUnitPriority(String unit, Ingredient ingredient) {
        ...
        if (category != null) {
            if (category == Ingredient.Category.SPICE || category == Ingredient.Category.SWEETENER) {
                if (normalizedUnit.equals("cay kasigi") || normalizedUnit.equals("teaspoon")) return 1;
                if (normalizedUnit.equals("tatli kasigi")) return 2;
                if (normalizedUnit.equals("g") || normalizedUnit.equals("gram")) return 3;
            }
            if (category == Ingredient.Category.OIL || category == Ingredient.Category.SAUCE || category == Ingredient.Category.BEVERAGE) {
                if (normalizedUnit.equals("ml")) return 1;
                if (normalizedUnit.equals("su bardagi") || normalizedUnit.equals("bardak") || normalizedUnit.equals("glass")) return 2;
                if (normalizedUnit.equals("yemek kasigi") || normalizedUnit.equals("tablespoon")) return 3;
            }
            if (category == Ingredient.Category.EGG) {
        ...
        return switch (normalizedUnit) {
            case "g", "gram" -> 5;
            case "ml" -> 6;
            case "su bardagi", "bardak", "glass" -> 7;
            case "adet", "unit", "piece", "tane" -> 8;
            case "l", "lt", "litre", "liter" -> 9;
            case "yemek kasigi", "tablespoon", "kasik" -> 10;
            case "dis", "clove" -> 11;
            case "tatli kasigi" -> 12;
            case "cay kasigi", "teaspoon" -> 13;
            case "fincan", "kahve fincani", "kepce" -> 14;
            case "tutam", "pinch", "dal", "demet" -> 15;
            case "kase", "bowl", "avuc", "handful" -> 16;
            case "kg", "kilogram" -> 17;
            default -> 20;
        };
    }
