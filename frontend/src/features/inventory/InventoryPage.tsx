  ...
  return (
    <>
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        {pageError && (
          <div className="rounded-[2rem] border border-red-200/70 dark:border-red-900/30 bg-red-50/90 dark:bg-red-900/10 px-5 py-4 text-red-700 dark:text-red-400 shadow-[0_18px_48px_-28px_rgba(185,28,28,0.35)] dark:shadow-none">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-red-800 dark:text-red-300">İşlem tamamlanamadı</p>
        ...
        {successMessage && (
          <div className="rounded-[2rem] border border-moss-sage/30 dark:border-emerald-900/30 bg-moss-sage/10 dark:bg-emerald-900/10 px-5 py-4 text-moss-forest dark:text-emerald-400 shadow-[0_18px_48px_-28px_rgba(74,93,78,0.35)] dark:shadow-none">
            <div className="flex items-start gap-3">
              <CheckCircle2 size={18} className="mt-0.5 shrink-0 text-moss-sage dark:text-emerald-500" />
              <div>
                <p className="font-semibold text-moss-forest dark:text-emerald-300">İşlem tamamlandı</p>
                <p className="mt-1 text-sm text-moss-forest/80 dark:text-emerald-400">{successMessage}</p>
              </div>
            </div>
          </div>
        )}
              ...
              return (
                <button
                  ...
                  className={`min-w-[220px] rounded-[2rem] border px-5 py-4 text-left transition-all ${
                    active ? 'border-transparent bg-terracotta text-white shadow-xl shadow-terracotta/25' : 'glass-card border-white/60 dark:border-white/10 dark:bg-gray-800/40 text-espresso-midnight dark:text-alabaster shadow-[0_18px_46px_-30px_rgba(40,36,33,0.35)] hover:border-moss-sage/30'
                  }`}
                >
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <section className="meal-card shadow-[0_24px_60px_-30px_rgba(40,36,33,0.38)]">
            ...
            <form id="inventory-item-form" onSubmit={handleItemSubmit} className="mt-6 space-y-5">
                    ...
                    <div className="absolute inset-x-0 top-[calc(100%+0.65rem)] z-20 overflow-hidden rounded-[1.75rem] border border-white/70 dark:border-gray-700 bg-alabaster dark:bg-gray-800 shadow-[0_24px_60px_-30px_rgba(40,36,33,0.45)]">
                      {searchingIngredients ? (
                        ...
                        ingredientResults.map((ingredient) => (
                          <button key={ingredient.id} type="button" onClick={() => handleIngredientSelect(ingredient)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-terracotta/8 dark:hover:bg-terracotta/10">
                            <div>
              ...
              <button type="submit" disabled={savingItem || !activeGroup} className="inline-flex w-full items-center justify-center gap-2 rounded-[1.6rem] bg-espresso-midnight dark:bg-terracotta px-5 py-4 font-bold text-white shadow-xl shadow-black/10 dark:shadow-none transition-all hover:bg-espresso-midnight/90 disabled:cursor-not-allowed disabled:opacity-60">
            ...
            <div className="meal-card shadow-[0_24px_60px_-30px_rgba(40,36,33,0.38)]">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button type="button" onClick={openEditLocationModal} disabled={!activeGroup} className="inline-flex items-center gap-2 rounded-2xl border border-espresso-midnight/10 dark:border-gray-700 bg-white/75 dark:bg-gray-800/40 px-4 py-3 text-sm font-semibold text-espresso-midnight/70 dark:text-gray-300 transition-all hover:text-terracotta disabled:cursor-not-allowed disabled:opacity-60">
                    <Pencil size={16} />
                    Düzenle
                  </button>
                  <button type="button" onClick={handleDeleteGroup} disabled={groups.length <= 1 || !activeGroup} className="inline-flex items-center gap-2 rounded-2xl border border-red-200 dark:border-red-900/30 bg-red-50/80 dark:bg-red-900/10 px-4 py-3 text-sm font-semibold text-red-600 dark:text-red-400 transition-all hover:bg-red-100 dark:hover:bg-red-900/20 disabled:cursor-not-allowed disabled:opacity-60">
                    <Trash2 size={16} />
                    Lokasyonu Sil
                  </button>
                </div>
                  ...
                  <article key={item.id} className="meal-card rounded-[2.2rem] border-white/60 p-5 shadow-[0_22px_56px_-32px_rgba(40,36,33,0.34)] hover:-translate-y-1 hover:shadow-[0_30px_70px_-30px_rgba(40,36,33,0.42)] dark:border-gray-800">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => handleEditItem(item)} className="rounded-full bg-white/80 dark:bg-gray-800/80 p-2 text-espresso-midnight/60 dark:text-gray-300 transition-colors hover:text-terracotta" aria-label="Malzemeyi düzenle"><Pencil size={15} /></button>
                    ...
                    <div className="meal-metric-card mt-6 px-4 dark:border-gray-800">
                      ...
                      <div className="mt-2 flex items-end gap-2">
                        ...
                        <span className="pb-1 text-sm font-semibold uppercase tracking-[0.18em] text-terracotta dark:text-terracotta/90">{item.unit}</span>
                      </div>
                    </div>
                  </article>
                ))}
      ...
      {locationModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-espresso-midnight/45 px-4 py-8 backdrop-blur-sm">
          <div className="w-full max-w-xl overflow-hidden rounded-[2.75rem] bg-alabaster dark:bg-gray-800 shadow-[0_38px_100px_-34px_rgba(40,36,33,0.72)] dark:shadow-none border dark:border-gray-700">
              ...
              <form onSubmit={handleLocationSubmit} className="mt-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {LOCATION_ICONS.map((option) => {
                    ...
                    return (
                      <button key={option.value} type="button" onClick={() => setGroupDraft((current) => ({ ...current, icon: option.value }))} className={`rounded-[1.8rem] border px-4 py-4 text-left transition-all ${selected ? 'border-transparent bg-terracotta text-white shadow-lg shadow-terracotta/20' : 'border-espresso-midnight/10 dark:border-gray-700 bg-white/70 dark:bg-gray-800/40 text-espresso-midnight/70 dark:text-gray-400 hover:border-moss-sage/30 hover:text-terracotta dark:hover:border-terracotta/50'}`}>
                        <div className="flex items-center gap-3">
                          <div className={`rounded-2xl p-3 ${selected ? 'bg-white/15' : 'bg-moss-sage/10 dark:bg-white/5 text-moss-forest dark:text-moss-sage'}`}><Icon size={18} /></div>
                          <div>
                            <p className="font-semibold">{option.label}</p>
                            <p className={`mt-1 text-[11px] uppercase tracking-[0.16em] ${selected ? 'text-white/70' : 'text-espresso-midnight/35 dark:text-gray-500'}`}>{option.value}</p>
                          </div>
                        </div>
                      </button>
                    );
                ...
                <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
                  <button type="button" onClick={closeLocationModal} className="rounded-2xl border border-espresso-midnight/10 dark:border-gray-700 bg-white/75 dark:bg-gray-800/40 px-5 py-3 font-semibold text-espresso-midnight/70 dark:text-gray-300 transition-colors hover:text-terracotta dark:hover:text-terracotta">Vazgeç</button>
                  <button type="submit" disabled={savingGroup} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-espresso-midnight dark:bg-terracotta px-5 py-3 font-bold text-white shadow-xl shadow-black/10 dark:shadow-none transition-all hover:bg-espresso-midnight/90 disabled:cursor-not-allowed disabled:opacity-60">
